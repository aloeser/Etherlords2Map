...
- without "Learning" proficiency, you can reach level 17 before fighting the final boss, and level 18/19 with "Learning"
...
- up to a certain point (lvl 14ish), monsters behave as you expect them to (typical spells, and only spells from their own race). Around level 14, however, you will trigger paleness. Monsters in pale areas may be more unfair and use spells from multiple races. There are a couple surprises hidden there
...
- skip-functionality: the map is intended to be difficult. To prevent players from getting stuck, the system will offer to remove (almost) any monster from the map if you fail to defeat it too often (3+ attempts). The skipping functionality is completely optional, you can try to defeat a monster as many times as you wish, and decide to skip it at any time
  - we recommend players to use this feature only as a last resort, as you will receive neither spells nor experience if you skip a monster
...
Your `Maps` folder should contain all the `Catch the Thief*`-files, and a folder called `CtT`.
...
Finally, even more thanks to Bonesnake for showing me how to access the logic of the fight itself, and beta-testing the resulting new monsters!
This is something I have wanted to do for a long time, because it opens up a lot of potential for interesting new mechanics, e.g., auto-casting a spell on a regular basis, or when some trigger-condition is fulfilled, etc.
...
## Skipping feature
The map contains some quite difficult monsters.
To prevent players from getting stuck, the system will offer to remove the monster from the map, if you fail to defeat it at least thrice.
This happens in the form of a yes/no dialog.
Press "yes" to remove the monster, and "no" to keep trying.
You will receive neither loot (spells) nor experience for monsters you remove, so you should not use it too often.
This feature works on all monsters except of one optional monster, the Konung, and the final three fights.
